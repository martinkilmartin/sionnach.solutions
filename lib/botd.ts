...
export async function botdEdge(
  ...
  switch (botdStatus) {
    ...
    case STATUS.PROCESSED: {
      ...
      res.cookie(COOKIE_NAME, requestId ?? 'null-request-id')
...
export async function botd(
  ...
  switch (status) {
    case STATUS.ERROR:
      ...
      headers.set(
        ERROR_DESCRIPTION_HEADER,
        botdRes.headers.get(ERROR_DESCRIPTION_HEADER) ?? ''
      )
      return
...
function getToken() {
  const token = process.env.BOTD_API_TOKEN

  if (!token) {
    if (process.env.NODE_ENV !== 'production') {
      throw new Error('BOTD_API_TOKEN is not defined')
    }
    console.error('Skipping Botd because the env BOTD_API_TOKEN is missing')
  }
...
export function isFavicon(req: NextRequest): boolean {
